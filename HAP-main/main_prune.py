...
from models.vgg import BottleneckVGG
...
def main(config):
    ...
    for it in range(len(epochs)):
        ...
        if config.init_test:
            train_loss_before_pruned, train_acc_before_pruned, train_top5_acc = pruner.test_model(trainloader, criterion, device)
            test_loss_before_pruned, test_acc_before_pruned, test_top5_acc = pruner.test_model(testloader, criterion, device)
            logger.info('Pretrain: Accuracy: %.2f%%(train), %.2f%%(test), %.2f%%(top5 train), %.2f%%(top5 test).'
                        % (train_acc_before_pruned, test_acc_before_pruned, train_top5_acc, test_top5_acc))
            logger.info('          Loss:     %.2f  (train), %.2f  (test).' % (train_loss_before_pruned, test_loss_before_pruned))
        ...
        test_loss_finetuned, test_acc_finetuned = pruner.fine_tune_model(trainloader=trainloader,
                                                                         ...
                                                                         weight_decay=wd,
                                                                         nepochs=epoch)
        train_loss_finetuned, train_acc_finetuned, train_top5_acc_finetuned = pruner.test_model(trainloader, criterion, device)
        _, _, test_top5_acc_finetuned = pruner.test_model(testloader, criterion, device)
        logger.info(f'After {config.dataset, config.network, config.depth}:  '
                    f'Accuracy: %.2f%%(train), %.2f%%(test), %.2f%%(top5 train), %.2f%%(top5 test) .'
                    % (train_acc_finetuned, test_acc_finetuned, train_top5_acc_finetuned, test_top5_acc_finetuned))
        ...
        stat = {
            ...
            'before_prune/test_acc':test_acc_before_pruned,
            'before_prune/test_acc_top5': test_top5_acc,
            'prune/test_loss': test_loss_pruned,
            'prune/test_acc': test_acc_pruned,
            'prune/test_acc_top5': top5_acc_pruned,
            ...
            'finetune/train_acc_top5': train_top5_acc_finetuned,
            'finetune/test_acc_top5': test_top5_acc_finetuned
        }
